...
class Main(tk.Frame):
    ...
    def init_main(self):
        ...
        self.add_img = tk.PhotoImage(file='D:img/add.png')
        ...
        self.tree.column("ID", width=38, anchor=tk.CENTER)
        self.tree.column("name", width=388, anchor=tk.CENTER)
        self.tree.column("tel", width=150, anchor=tk.CENTER)
        self.tree.column("email", width=158, anchor=tk.CENTER)

        self.tree.heading("ID", text='ID')
        self.tree.heading("name", text='Name')
        self.tree.heading("tel", text='Telephone')
        self.tree.heading("email", text='E-mail')
        self.tree.pack(side=tk.LEFT)

        self.update_img = tk.PhotoImage(file='D:img/update.png')
        btn_edit_dialog = tk.Button(toolbar, bg = "#d7d8e0",
                                    bd = 0, image=self.update_img,
                                    command=self.open_update_dialog)
        btn_edit_dialog.pack(side=tk.LEFT)

        self.delete_img = tk.PhotoImage(file='D:img/delete.png')
        btn_delete = tk.Button(toolbar, bg='#d7d8e0', bd=0,
                               image=self.delete_img,
                               command=self.delete_records)
        btn_delete.pack(side=tk.LEFT)

        self.search_img = tk.PhotoImage(file='D:img/search.png')
        btn_search = tk.Button(toolbar, bg='#d7d8e0', bd=0,
                               image=self.search_img,
                               command=self.open_search_dialog)
        btn_search.pack(side=tk.LEFT)

        self.refresh_img = tk.PhotoImage(file='D:img/refresh.png')
...
#                             Я НЕ СМОГ ДОСТАТЬ ОРИГИНАЛЬНЫЕ ИКОНКИ ВЗЯЛ ИЗ ИНЕТА
